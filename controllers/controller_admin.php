...
class Controller_Admin
{
    ...
    {
        $info = new Model_Admin();
        $grade_id = isset($_POST['grade_id']) ? Htmlspecialchars(addslashes($_POST['grade_id'])) : null;
        echo json_encode($info->statistics($grade_id));
    }
    public function statistics_score()
    {
        $info = new Model_Admin();
        $grade_id = isset($_POST['grade_id']) ? Htmlspecialchars(addslashes($_POST['grade_id'])) : null;
        echo json_encode($info->statistics_score($grade_id));
    }
    ...
    {
        $list_subject_head = new Model_Admin();
        echo json_encode($list_subject_head->get_list_subject_head());
    }
    public function edit_subject_head($subject_head_id, $password, $name, $gender_id, $birthday, $subject_id)
    {
        $edit = new Model_Admin();
        return $edit->edit_subject_head($subject_head_id, $password, $name, $gender_id, $birthday, $subject_id);
    }
    public function del_subject_head($subject_head_id)
    {
        $del = new Model_Admin();
        return $del->del_subject_head($subject_head_id);
    }
    public function add_subject_head($name, $username, $password, $email, $birthday, $gender, $subject_id)
    {
        $add = new Model_Admin();
        return $add->add_subject_head($name, $username, $password, $email, $birthday, $gender, $subject_id);
    }
    ...
    public function add_question($subject_id, $question_content, $level_id, $grade_id, $unit, $answer_a, $answer_b, $answer_c, $answer_d, $correct_answer, $teacher_id)
    {
        $add_question = new Model_Admin();
        return $add_question->add_question($subject_id, $question_content, $level_id, $grade_id, $unit, $answer_a, $answer_b, $answer_c, $answer_d, $correct_answer, $teacher_id, '3');
    }
    ...
    public function get_dashboard_info()
    {
        ...
        $subject_head = new stdclass();
        $subject_head->count = $get_total->get_total_subject_head();
        $subject_head->name = "Trưởng bộ môn";
        $subject_head->icon = "fa-user";
        $subject_head->actionlink = "show_subject_head_panel";
        ...
        $statistic->count = $get_total->get_total_statistic();
        $statistic->name = "Thống kê";
        $statistic->icon = "fa-edit";
        $statistic->actionlink = "show_statistic_panel";
        $test = new stdclass();
        $test->count = $get_total->get_total_test();
        $test->name = "Bài Thi";
        $test->icon = "fa-edit";
        $test->actionlink = "show_tests_panel";
        $total = array($admin,$subject_head,$teacher,$student,$grade,$class,$subject,$question,$test,$statistic);
        return $total;
    }
    ...
    public function check_add_subject_head()
    {
        $result = array();
        $name = isset($_POST['name']) ? Htmlspecialchars(addslashes($_POST['name'])) : '';
        $username = isset($_POST['username']) ? Htmlspecialchars(addslashes($_POST['username'])) : '';
        ...
        if (empty($name)||empty($username)||empty($password)) {
           ...
           // var_dump($subject_id); die;
            $add = $this->add_subject_head($name, $username, $password, $email, $birthday, $gender, $subject_id);
            if ($add) {
    ...
    public function check_add_subject_head_via_file()
    {
        $inputFileType = 'Xlsx';
        $result = array();
        $reader = IOFactory::createReader($inputFileType);
    ...
    public function check_del_subject_head()
    {
        $result = array();
        $subject_head_id = Htmlspecialchars($_POST['subject_head_id']);
        $del = $this->del_subject_head($subject_head_id);
        if ($del) {
            $result['status_value'] = "Xóa thành công!";
            $result['status'] = 1;
            $result['subject_head_id'] = $subject_head_id;
        } else {
            $result['status_value'] = "Không thể xóa!";
            $result['status'] = 0;
            $result['subject_head_id'] = $subject_head_id;
    ...
    public function check_edit_subject_head()
    {
        $result = array();
        $subject_head_id = isset($_POST['subject_head_id']) ? Htmlspecialchars($_POST['subject_head_id']) : '';
        $name = isset($_POST['name']) ? Htmlspecialchars(addslashes($_POST['name'])) : '';
        $username = isset($_POST['username']) ? Htmlspecialchars(addslashes($_POST['username'])) : '';
        $gender_id = isset($_POST['gender_id']) ? Htmlspecialchars(addslashes($_POST['gender_id'])) : '';
        $birthday = isset($_POST['birthday']) ? Htmlspecialchars(addslashes($_POST['birthday'])) : '';
        ...
        //var_dump(($_POST[_dump
        if (empty($name)||empty($gender_id)||empty($birthday)||empty($subject_head_id)||empty($subject_id)) {
            $result['status_value'] = "Không được bỏ trống các trường nhập!";
            $result['status'] = 0;
        } else {
            if($_POST['password'] == ""){
            ...
            $update = $this->edit_subject_head($subject_head_id, $password, $name, $gender_id, $birthday, $subject_id);
            if (!$update) {
                $result['status_value'] = "Tài khoản không tồn tại!";
                $result['status'] = 0;
            } else {
    ...
    public function check_add_question()
    {
        ...
        if (empty($question_detail)||empty($grade_id)||empty($level_id)||empty($unit)||empty($subject_id)||empty($answer_a)||empty($answer_b)||empty($answer_c)||empty($answer_d)||empty($correct_answer)) {
            ...
            $res = $this->add_question($subject_id, $question_detail, $level_id, $grade_id, $unit, $answer_a, $answer_b, $answer_c, $answer_d, $answer, $teacher_id);
            if ($res) {
                $result['status_value'] = "Thêm thành công!";
                $result['status'] = 1;
            } else {
    ...
    public function check_add_question_via_file()
    {
        ...
        for ($i = 4; $i < count($sheetData); $i++) {
            ...
            $add = $this->add_question($subject_id, $question_content, $level_id, $grade_id, $unit, $answer_a, $answer_b, $answer_c, $answer_d, $answer, $teacher_id);
            if ($add) {
                $count++;
            } else {
                $err_list += $stt.' ';
    ...
    public function check_edit_question()
    {
        ...
        if (empty($question_content)||empty($grade_id)||empty($unit)||empty($level_id)||empty($answer_a)||empty($answer_b)||empty($answer_c)||empty($answer_d)||empty($correct_answer)) {
            ...
            $res = $this->edit_question($question_id, $subject_id, $question_content, $level_id, $grade_id, $unit, $answer_a, $answer_b, $answer_c, $answer_d, $answer,$suggest,$status_id);
            if ($res) {
                $result['status_value'] = "Sửa thành công!";
                $result['status'] = 1;
            } else {
    ...
    public function delete_check_subject_head()
    {
        $result = array();
        $list_del = "";
        $data = $_POST['list_check'];
        $list_check = explode(',', $data);
        for ($i = 0; $i < count($list_check) - 1; $i++) {
            $del = $this->del_subject_head($list_check[$i]);
            if (!$del) {
                $list_del = $list_del." ".$list_check[$i];
            }
        }
